...
After a job completes, compare what you requested vs what you actually used:

```bash
sacct -j JOBID --format=JobID,AllocCPUS,CPUTime,MaxRSS,ReqMem,Elapsed
```

- **AllocCPUS** — CPUs you requested · **MaxRSS** — peak RAM the job actually used · **ReqMem** — RAM you requested

Did you over-request memory? Use these numbers to calibrate your next job's `--mem` and `--cpus-per-task`.

<label class="quest-check"><input type="checkbox" data-room="d3-slurm-job" data-key="audit"> I audited my resource usage and know whether I over- or under-requested</label>

**Side quest — Watch a job's output live**

While a job is running (status `R`), follow its output as it writes:

```bash
tail -f logs/extract_JOBID.out
```

`Ctrl-C` to stop following. Useful for long jobs where you want progress without waiting for completion.

<label class="quest-check"><input type="checkbox" data-room="d3-slurm-job" data-key="tail"> I followed a running job's output live with tail -f</label>
...
<label class="quest-check"><input type="checkbox" data-room="d3-slurm-job" data-key="exit"> I decoded my job's ExitCode with sacct and can explain what it means</label>

**Side quest — Trigger an OOM on purpose**

Edit a job's `#SBATCH --mem` down to something clearly too small (e.g. `--mem=10M`) and resubmit. Watch it get killed for running out of memory, then compare its `sacct` `State` and `.err` signature to the bug you fixed earlier — can you tell an OOM kill apart from a code bug at a glance now?

<label class="quest-check"><input type="checkbox" data-room="d3-slurm-job" data-key="oom"> I deliberately triggered an OOM kill and compared its failure signature to a code bug</label>
...
It gets killed at the 10-second limit:

```bash
sacct -j JOBID --format=JobID,State,Elapsed,ExitCode
```

The `State` shows `TIMEOUT`. You've now seen three distinct failure fingerprints — a **code bug** (Python traceback), an **OOM kill** (`ExitCode 137`), and a **timeout** (`State TIMEOUT`) — and can tell them apart at a glance.

<label class="quest-check"><input type="checkbox" data-room="d3-slurm-job" data-key="timeout"> I deliberately triggered a timeout and can tell TIMEOUT, OOM, and code-bug failures apart</label>
