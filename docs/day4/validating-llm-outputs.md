...
Hallucinations can be professionally consequential in high-stakes settings. A memorable real-world case: in 2024, Stanford misinformation expert Jeff Hancock submitted expert testimony citing journal articles that ChatGPT had invented, and the court [threw it out](https://minnesotareformer.com/2024/12/02/misinformation-expert-used-ai-to-draft-testimony-containing-misinformation-about-ai/). (Lawyers have been sanctioned for the same thing.)
...
The stakes rise sharply when an LLM's output *drives an action* — writing to a database, sending emails, deleting files. LLMs make mistakes, and mistakes that change state are the most expensive kind: you often can't undo them. Recall from [Day 1](../../day1/command-spire/) that `rm` deletes permanently — no trash, no undo; an agent runs the same commands, just without a human pausing to reconsider.

A cautionary case: in early 2026 a user asked Claude to organize a desktop, and it deleted a folder holding roughly 15 years of family photos — thousands of files — with terminal commands that bypassed the Trash entirely. Agents run with *your* full permissions, so "clean up this folder" can reach anything you can.
...
<p style="color:#cc0000; font-weight:700; margin:1.5rem 0;">🚧 TODO: add the validation rules of thumb (spot-checking, comparing across models, format sanity checks), and consider any remaining OWASP failure modes (e.g. runaway loops); see the mapping in <code>TODO.md</code>.</p>
