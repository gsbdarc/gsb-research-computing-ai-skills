...
{: .note }
> 🔄 **Keep the leaderboard live.** In your terminal on the Yens, inside `~/rf-bootcamp-2026` — start Claude Code with `claude` if it isn't already running — tell it: "Look at the `DAYS` list in `docs/assets/js/quest-log.js` for the exact `room.key` names for Day 4, and in `quest_log.json` at my repo root (create it if missing) set to `true` every key for what I finished today — [list what you completed, e.g. the parallelization and SLURM-arrays quests, the array exercise, running an LLM, and this capstone]. Before pushing, run `git remote -v` and confirm `origin` is my own fork (`{{ site.data.site_meta.github_owner }}/rf-bootcamp-2026`), not the class repo `gsbdarc/rf-bootcamp-2026` — if it points to the class repo, stop and tell me. Then commit and push to `main`." Claude runs the `git add`/`commit`/`push` for you — same `main` branch you've been pushing to all along.
